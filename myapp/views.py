    ...
    if request.method == 'POST':
        ...
        if bus.rem > int(seats_r):
                ...
                book=Book.objects.create(name=username_r, email=email_r, userid=userid_r, bus_name=name_r,
                                           source=source_r, busid=id_r,dest=dest_r, price=price_r, nos=seats_r, date=date_r, time=time_r,                                           status="BOOKING CONFIRMED ")
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        user = User.objects.create_user(email_r)
        if user:
            login(request, user)
